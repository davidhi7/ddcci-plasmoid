...
def ddcutil_mock(request, default_command_output):
    """
...
async def test_detect(monkeypatch, ddcutil_mock):
    ...
    monkeypatch.setattr('ddcci_plasmoid_backend.subprocess_wrappers.subprocess_wrapper', sync_ddcutil_mock)
    monkeypatch.setattr('ddcci_plasmoid_backend.subprocess_wrappers.async_subprocess_wrapper', async_ddcutil_mock)
...
async def test_set_basic():
    ...
    with mock.patch.object(adapter._ddcutil_wrapper, 'run_async') as mocked_fn:
        ...
        mocked_fn.assert_called_once_with('setvcp', hex(ddcci_adapter.FeatureCode.BRIGHTNESS.value), '100', bus=0,
