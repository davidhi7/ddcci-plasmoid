...
from ddcci_plasmoid_backend.adapters.monitor_adapter import ContinuousValue, NonContinuousValue, Property
...
async def ddcutil_detect_monitors(ddcutil_wrapper: DdcutilWrapper) -> list[ddcci_adapter.DdcciMonitor]:
    """
    ...
    for monitor_node in parsed_output.children:
        ...
        monitor_tasks.append(asyncio.create_task(_gather_monitor_data(ddcutil_wrapper, ddcutil_id, monitor_node)))
    ...
    logger.info(f'Detected {count_filtered} working monitor {"bus" if count_filtered == 1 else "busses"}, '
                f'{count_remaining} malfunctioning {"bus" if count_remaining == 1 else "busses"}.')
    return monitors


async def _gather_monitor_data(ddcutil_wrapper: DdcutilWrapper, ddcutil_id: int,
                               monitor: Node) -> ddcci_adapter.DdcciMonitor:
    """
    ...
    if ddcci_adapter.FeatureCode.BRIGHTNESS.value in capabilities:
        vcp_values[Property.BRIGHTNESS] = \
            ContinuousValue(
                value=await _get_vcp_value(ddcutil_wrapper, bus_id, ddcci_adapter.FeatureCode.BRIGHTNESS.value),
                min_value=0, max_value=100)
    if ddcci_adapter.FeatureCode.CONTRAST.value in capabilities:
        vcp_values[Property.CONTRAST] = \
            ContinuousValue(
                value=await _get_vcp_value(ddcutil_wrapper, bus_id, ddcci_adapter.FeatureCode.CONTRAST.value),
                min_value=0, max_value=100)
    if ddcci_adapter.FeatureCode.POWER_MODE.value in capabilities:
        vcp_values[Property.POWER_MODE] = NonContinuousValue(
            value=await _get_vcp_value(ddcutil_wrapper, bus_id, ddcci_adapter.FeatureCode.POWER_MODE.value),
...
async def _get_vcp_value(ddcutil_wrapper: DdcutilWrapper, bus: int, feature_code: int) -> int:
    """
    ...
    """
    result = await ddcutil_wrapper.run_async('getvcp', '--brief', hex(feature_code), bus=bus, logger=logger)
...
async def _parse_capabilities(ddcutil_wrapper: DdcutilWrapper, bus: int) -> ddcci_adapter.VcpFeatureList:
    """
    ...
    output = await ddcutil_wrapper.run_async('capabilities', '--brief', bus=bus, logger=logger)
