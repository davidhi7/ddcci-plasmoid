...
class DdcutilWrapper:
    ...
    async def run_async(self, verb: str, *arguments: str, bus: int | None = None,
                        logger: logging.Logger) -> subprocess_wrappers.CommandOutput:
        """
        ...
        while re.findall(ddcutil_failure_pattern, output.stdout):
            if attempt == self.brute_force_attempts:
                ...
                if attempt > 0:
                    logger.warning(f'Command `{command}` failed {attempt + 1} times in a row')
                break
            attempt += 1
            output = await subprocess_wrappers.async_subprocess_wrapper(command, logger)
        else:
    ...
    def _build_command(self, verb: str, *arguments: str, bus: int | None) -> str:
        ...
        if self.sleep_multiplier and self.sleep_multiplier != DDCUTIL_DEFAULT_SLEEP_MULTIPLIER:
    ...
    def _strip_ddcutil_nvidia_warning(
            command_output: subprocess_wrappers.CommandOutput) -> subprocess_wrappers.CommandOutput:
        """
    ...
    def get_ddcutil_version(self) -> str:
        ...
        try:
            ...
            output = subprocess_wrappers.subprocess_wrapper(f'{self.ddcutil_executable} --version',
